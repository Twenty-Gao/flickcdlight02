...
class Trainer(object):
    ...
    def training(self):
        ...
        # 蒸馏权重（Alpha 控制软硬目标的平衡）
        # 蒸馏权重 (Alpha 控制软硬目标的平衡)
        alpha = self.args.distill_alpha if hasattr(self.args, 'distill_alpha') else 0.7
        ...
        for e in range(start_epoch, self.epoch):
            ...
            for iter, data in enumerate(progress_bar):
                ...
                s_feat = outputs['student_feature']
                t_feat = outputs['teacher_feature']
                masks = outputs['mask']

                # === 1. 计算 Soft Loss (特征蒸馏) ===
                # 归一化以稳定数值
                s_norm = F.normalize(s_feat, p=2, dim=1, eps=1e-8)
                t_norm = F.normalize(t_feat, p=2, dim=1, eps=1e-8)
                loss_soft = mse_loss_fn(s_norm, t_norm) * 10.0

                # === 2. 计算 Hard Loss (任务监督) ===
                # 计算主输出 mask1 的 BCE Loss
