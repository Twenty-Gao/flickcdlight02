...
class FlickCD(nn.Module):
    ...
    def forward(self, t1, t2,return_all=False):
        ...
        if return_all:
            with torch.no_grad():
                t1_feats = self.teacher_backbone(t1)
                t1_last_feat = t1_feats[2]  # Teacher Stage 3 [B, 160, H/32, W/32]

                t2_feats = self.teacher_backbone(t2)
                t2_last_feat = t2_feats[2]
        ...
        s2_3_adapted = self.adapter(s2_3)

        # --- 3. 变化检测解码 (Hard Target 生成) ---
        # 将学生提取的特征输入 UCM 和 Decoder
        stage1, stage2, stage3 = self.ucm(s1_1, s1_2, s1_3_adapted, s2_1, s2_2, s2_3_adapted)  # 注意这里用了 adapted
        ...
        if return_all:
            # 训练模式：同时返回“特征”和“掩码”
            return {
                'teacher_feature_t1': t1_last_feat,
                'teacher_feature_t2': t2_last_feat,  # 教师特征 (Target for Soft Loss)
                'student_feature_t1': s1_3_adapted,  # 学生特征 (Source for Soft Loss)
                'student_feature_t2': s2_3_adapted,
