...
from model.encoder_distillation import repvit_m0, repvit_m2_3, repVgg, lightVGG
...
class FlickCD(nn.Module):
    ...
    def forward(self, t1, t2=None,return_features=True):
        # --- 教师前向 (只用于提取特征做目标) ---
        with torch.no_grad():
            # RepViT 返回的是一个列表或元组，通常最后一个是深层特征
            # 根据你的 repvit_m0 定义，它返回三个阶段的特征
            t_feats = self.teacher_backbone(t1)
            # 取最后一个特征图 (Stage 3)，通道数 160
            t_last_feat = t_feats[2]

        # --- 学生前向 ---
        # 假设 VGG 修改后返回最后一层特征 [B, 512, H, W]
        s_feat = self.student_backbone(t1) # [B, 512, H/32, W/32]
        # --- 特征适配 ---
        # [B, 512, ...] -> [B, 160, ...]
        # s_adapted = self.adapter(s_feat)
        # 让学生特征经过适配层
        s_feat_adapted = self.adapter(s_feat)
        # 返回字典结构，方便计算 Loss
        if return_features:
            return {'teacher': t_last_feat, 'student': s_feat_adapted}

        return s_feat_adapted
