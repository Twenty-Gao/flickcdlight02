...
class LightVGG(nn.Module):
    ...
    def forward(self, x):
        output = []
        # 指定需要提取特征的层索引 (根据上面的 nn.Sequential 定义)
        # 我们需要在 MaxPool 之后提取特征
        # Stage 1 out (64x64): idx 3
        # Stage 2 out (32x32): idx 5
        # Stage 3 out (16x16): idx 7 或者 idx 8 (最后输出)

        target_indices = [3, 5, 8]

        for idx, layer in enumerate(self.features):
            x = layer(x)
            # print(f"Layer {idx} shape: {x.shape}") # 调试用
        ...
        # output[0]: [B, 48, 64, 64]
        # output[1]: [B, 96, 32, 32]
        # output[2]: [B, 160, 16, 16]
        return output[0], output[1], output[2]


class LightVGG_Pro(nn.Module):
    def __init__(self):
        super(LightVGG_Pro, self).__init__()

        # 目标：参数量对齐 RepViT-nano (~1.29M)
        # 策略：相比原 LightVGG，加宽通道数 (Width Scaling)
        # 原通道: [24, 48, 96, 160] -> 约 0.97M
        # 新通道: [32, 64, 128, 216] -> 预计约 1.3M - 1.4M 左右

        self.features = nn.Sequential(
            # --- Stage 0: 256x256 -> 128x128 ---
            # 普通卷积
            nn.Sequential(
                nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=1, bias=False),  # 24 -> 32
                nn.BatchNorm2d(32),
                nn.ReLU(inplace=True)
            ),  # idx 0
            nn.MaxPool2d(kernel_size=2, stride=2),  # idx 1

            # --- Stage 1: 128x128 -> 64x64 ---
            DSConv(32, 64),  # idx 2  (24->48 改为 32->64)
            nn.MaxPool2d(kernel_size=2, stride=2),  # idx 3 -> Output[0]

            # --- Stage 2: 64x64 -> 32x32 ---
            DSConv(64, 128),  # idx 4 (48->96 改为 64->128)
            nn.MaxPool2d(kernel_size=2, stride=2),  # idx 5 -> Output[1]

            # --- Stage 3: 32x32 -> 16x16 ---
            DSConv(128, 216),  # idx 6 (96->160 改为 128->216)
            nn.MaxPool2d(kernel_size=2, stride=2),  # idx 7 -> Output[2]

            # --- Stage 4: 保持 16x16 ---
            # 这一层参数量最大，对总参数量影响最大
            DSConv(216, 216)  # idx 8 (160->160 改为 216->216)
        )
...
def repvit_student02(pretrained=False, num_classes=1000, distillation=False):
    """
    修改后的学生模型：
    1. 通道数与 m0 保持一致 (40, 80, 160)，方便直接蒸馏。
    2. 深度大幅减少，尤其是 Stage 2。
    """
    cfgs = [
        # k, t, c, SE, HS, s
        # k=kernel_size, t=expand_ratio, c=channel, s=stride

        # --- Stage 0 (40 channels) ---
        # 教师模型这里有2层，学生保持2层，保证基础特征提取能力
        [3, 2, 40, 1, 0, 1],
        [3, 2, 40, 0, 0, 1],  # 累计层数: 2 (Index 1) -> Output[1]

        # --- Stage 1 (80 channels) ---
        # 教师模型这里有3层，学生保留3层 (1个下采样 + 2个处理)
        # 如果想更极致轻量，可以去掉最后一行
        [3, 2, 80, 0, 0, 2],  # Downsample
        [3, 2, 80, 1, 0, 1],  # 累计层数: 4 (Index 3) -> Output[3]

        # --- Stage 2 (160 channels) ---
        # 教师模型这里有 10+ 层！这是计算量最大的地方。
        # 学生模型我们只保留 3 层 (1个下采样 + 2个处理)，大幅加速。
        [3, 2, 160, 0, 1, 2],  # Downsample
        [3, 2, 160, 1, 1, 1], # 累计层数: 6 (Index 5) -> Output[-1]
    ]
...
def repvit_student01(pretrained=False, num_classes=1000, distillation=False):
    ...
    cfgs = [
        ...
        [3, 2, 80, 0, 0, 2],  # Downsample
        [3, 2, 80, 1, 0, 1],
        [3, 2, 80, 0, 0, 1],  # 累计层数: 5 (Index 4) -> Output[4]
        ...
        [3, 2, 160, 0, 1, 2],  # Downsample
        [3, 2, 160, 1, 1, 1],
        [3, 2, 160, 0, 1, 1],  # 累计层数: 8 (Index 7) -> Output[-1]
    ]
    return RepViT(cfgs, num_classes=num_classes, distillation=distillation)
...
def lightVGG_pro(pretrained=False, num_classes=10, distillation=False):
    return LightVGG_Pro()
