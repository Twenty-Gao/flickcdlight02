...
#
#     path = "./model/repvit_m0_6.pth"
    # path = "./result/flickcd_student03.pth"
    # path = "./result/flickcd_student04.pth"
    # path = "./result/flickcd_student04.pth"
    # path = "./result/LEVIR_CD_Experiment_distillation18/best_student_model.pth"
    ...
    #     # 兼容 FlickCD 保存格式 (student_backbone.) 或单独保存格式
    #     if k.startswith('student_backbone.'):
    #         new_state_dict[k] = v
    #     elif k.startswith('adapter.'):
    #         new_state_dict[k] = v
    #     elif k.startswith('features.'):  # 纯 VGG 权重
    #         new_state_dict['student_backbone.' + k] = v
    #
    # #只有当键名完全一致，但形状不一致时，才会报错。
    ...
    # new_state_dict = {}
    # for k, v in state_dict.items():
    #     # 情况1: 如果权重文件里包含 'student_backbone.' 前缀，需要去掉它
    #     # 因为我们现在是直接给 backbone 加载权重，而不是给整个大模型加载
    #     if k.startswith('student_backbone.'):
    #         new_key = k.replace('student_backbone.', '')
    #         new_state_dict[new_key] = v
    #
    #     # 情况2: 如果是纯 adapter 权重，通常 backbone 不需要加载它，这里根据你的需求决定是否保留
    #     # 如果 backbone 里面没有 adapter 层，strict=True 会报错，所以通常不加
    #     elif k.startswith('adapter.'):
    #         continue
    #
    #         # 情况3: 如果是通用特征提取层 (视具体 backbone 定义而定)
    ...
    #
    # # 打印加载了多少层，方便调试
    # print(f"Constructed state_dict with {len(new_state_dict)} keys.")
    #
    # # 此时开启 strict=True，要求 new_state_dict 的键必须与 model 完美匹配
